...
// Uniwersalna inicjalizacja przycisków interfejsu użytkownika
...
// Uniwersalna inicjalizacja pasków postępu (HUD)
...
// Pełne resetowanie stanu gry i powrót do wartości początkowych
    ...
    , mVictoryAnimTimer(0.0f)
{
    ...
    // Ładowanie podstawowych zasobów graficznych i czcionek
    ...
    // Konfiguracja elementów interfejsu (Menu, HUD, Ekrany Końcowe)
    ...
    // Konfiguracja efektów wizualnych dla ekranu zwycięstwa
    ...
    // Ładowanie zasobów podglądu przedmiotów w sklepie
    for (int i = 0; i < 3; ++i) {
    ...
    // Inicjalizacja puli przeciwników na ekranie gry
    for (int i = 0; i < 5; ++i) {
...
// Losowanie typu, pozycji oraz prędkości przeciwnika (Ptak, Dron lub Krowa)
...
// Aktualizacja parametrów fizycznych opony i tekstur po zakupie ulepszenia
...
// Główna pętla obsługi zdarzeń (sterowanie, kliknięcia w menu i sklepie)
void Game::processEvents() {
    ...
    while (mWindow.pollEvent(event)) {
        ...
        // Obsługa interakcji na ekranie zwycięstwa
        ...
        // Zmiana stanów gry za pomocą klawiszy numerycznych
        ...
        // Logika mechaniki QTE (wybór siły i kąta wystrzału opony)
        ...
        // Obsługa kliknięć myszką w Menu Głównym i Sklepie (wybór / zakup ulepszeń)
        if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
            sf::Vector2i pixelPos = sf::Mouse::getPosition(mWindow);
            sf::Vector2f mousePos = mWindow.mapPixelToCoords(pixelPos);
...
// Główna logika aktualizacji stanu gry (fizyka, kolizje, animacje)
void Game::update(sf::Time deltaTime) {
    if (mCurrentState != GameState::PLAY) return;

    float dt = deltaTime.asSeconds();

    // Animacja przejścia i efektów świetlnych dla ekranu zwycięstwa
    if (mIsVictoryScreenActive) {
    ...
    // Sekwencja końca gry (efekt przyciemnienia i pulsowanie tekstu)
    ...
    // Obsługa ostrzeżenia przed pojawieniem się Bossa (czerwony błysk)
    ...
    // Aktualizacja pasków postępu celów i uderzeń krów
    ...
    // Obsługa klatek animacji świętowania postaci po udanym trafieniu
    ...
    // Aktualizacja pozycji przeciwników i respawn po wylocie za ekran
    for (size_t i = 0; i < mTargets.size(); ++i) {
    ...
    if (mRocket.isFlying()) {
        ...
        // Skalowanie hitboxa opony dla dokładniejszego wykrywania kolizji
        ...
        // Iteracja po celach i przetwarzanie rezultatów trafień (obrażenia, śmierć, leczenie Bossa)
        for (size_t i = 0; i < mTargets.size(); ++i) {
            if (mTargets[i]->isActive() && smallTireBox.intersects(mTargets[i]->getBounds())) {
                ...
                if (hitRes == HitResult::IGNORED) {
                    ...
                    if (mTargets[i]->isBoss()) {
                        ...
                        // Specjalna logika dla trafienia krowy (leczenie bossa lub wywołanie Game Over)
    ...
    // Automatyczny zapis postępu w momencie wylądowania opony
    bool currentlyFlying = mRocket.isFlying();
    if (mWasFlying && !currentlyFlying) saveProgress();
    mWasFlying = currentlyFlying;
}

// Renderowanie całej sceny w zależności od aktualnego stanu maszyny stanów gry
void Game::render() {
    sf::Color bgColor;
    ...
    // RENDEROWANIE: Główny stan rozgrywki (Ulica / Poziom gry)
        ...
        // Dynamiczne rysowanie punktów przewidywanej trajektorii opony
        ...
        // Rysowanie graficznego interfejsu skali QTE (strefy zielona/żółta/czerwona)
        if (mQteManager.isActive()) {
            float barW = 400.0f;
            float barH = 30.0f;
        ...
        // Dynamiczne ładowanie i wyświetlanie licznika monet na HUD w trakcie gry
        ...
        // Efekty specjalne nakładane na ekran w przypadku pokonania bossa (zwycięstwo)
        if (mIsVictoryScreenActive) {
            float t = mVictoryAnimTimer / 1.5f;
            if (t > 1.0f) t = 1.0f;
            float smoothT = std::sin(t * 3.14159f / 2.0f);
...
// Zapis aktualnych postępów gracza (monety, ulepszenia) do pliku tekstowego
...
// Odczyt zapisanych stanów gry z pliku tekstowego na starcie aplikacji
